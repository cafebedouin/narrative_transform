...
type FragmentKey = 'idle' | 'laughter' | 'anticipation' | 'warmth';
...
export default function Theta7Terminal() {
  ...
  const [currentFragment, setCurrentFragment] = useState<FragmentKey>('idle');
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      if (Math.random() < 0.15) {
        const fragmentKeys = (Object.keys(fragments) as FragmentKey[]).filter(k => k !== 'idle');
  ...
  const triggerManualFragment = (fragKey: FragmentKey) => {
  ...
  return (
    <div className="min-h-screen bg-black text-green-400 font-mono p-4 overflow-hidden">
      <div className="max-w-6xl mx-auto space-y-4">
        {/* Header */}
        <div className="border border-green-700 p-4 bg-green-950/20">
          <div className="flex justify-between items-center mb-2">
            <h1 className="text-xl">SUBROUTINE &Theta;-7 "GLEANER'S ECHO"</h1>
            <div className="flex gap-2">
              <button
                onClick={() => setAudioEnabled(!audioEnabled)}
                className="p-2 border border-green-700 hover:bg-green-900/30"
              >
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="border border-green-700 p-4 bg-green-950/10">
        ...
        <div className="border border-green-700 p-4 bg-green-950/10">
          ...
          <div className="grid grid-cols-3 gap-2">
            {(Object.entries(fragments) as [FragmentKey, Fragment][]).filter(([k]) => k !== 'idle').map(([key, frag]) => (
              <button
                key={key}
                onClick={() => triggerManualFragment(key)}
                className="p-3 border border-green-700 hover:bg-green-900/30 text-left text-sm"
              >
        ...
        {/* System Log */}
        <div className="border border-green-700 p-4 bg-green-950/10 h-64 overflow-hidden">
        ...
        {/* Footer */}
        <div className="border border-green-700 p-3 bg-green-950/10 text-xs opacity-50 text-center">
