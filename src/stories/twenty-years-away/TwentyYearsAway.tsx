...
interface SystemState {
  ...
  hysteresisFlags: {
    ...
    metric_trust_eroded: boolean;
  };
  terminalReached: boolean;
  scrubPosition: number;
  syncPoint: string;
  omegaRevealed: boolean[];
  probeCount: number;
  idleTicks: number;
}
...
interface AppState {
  ...
  uczF: UczF;
  transformationRules: {
    TR1: TransformationRule;
    TR2: TransformationRule;
    TR3: TransformationRule;
    TR4: TransformationRule;
  };
  system: SystemState;
}
...
function getSyncPoint(scrub: number): string {
...
function propagateCouplings(state: AppState): AppState {
...
interface ContentBlock {
  ...
  c3Note: string;
  mood: string;
  metrics?: { employment: number; housing: number; socialNet: number; reintegration: number };
}

const LITERARY_CONTENT: Record<string, ContentBlock> = {
  SP1: {
...
const REENTRY_CONTENT: Record<string, ContentBlock> = {
  SP1: {
    title: "Before",
...
interface MetricBarProps {
  ...
  value: number;
  ghosted: boolean;
  hysteresisNote?: string | null;
  betrayed?: boolean;
}

const MetricBar = ({ label, value, ghosted, hysteresisNote, betrayed }: MetricBarProps) => {
...
const OmegaCard = ({ text, revealed, onProbe }: OmegaCardProps) => (
  <div
    onClick={onProbe}
    style={{
...
interface ConstraintHighlightProps {
  type: string;
  active: boolean;
  children: React.ReactNode;
}

const ConstraintHighlight = ({ type, active, children }: ConstraintHighlightProps) => {
...
export default function TwentyYearsAway() {
  ...
  const idleTimerRef = useRef<ReturnType<typeof setTimeout> | ReturnType<typeof setInterval> | null>(null);
  const scrubRef = useRef<HTMLDivElement>(null);

  const dispatch = useCallback((action: string, payload?: number) => {
    setState((prev) => {
      ...
      switch (action) {
        ...
        case "revealOmega": {
          if (payload !== undefined && payload >= 0 && payload < 3 && !next.system.omegaRevealed[payload]) {
  ...
  useEffect(() => {
    if (hasInteracted) {
      if (idleTimerRef.current) clearInterval(idleTimerRef.current as ReturnType<typeof setInterval>);
    ...
    >
      ...
      <div style={{ padding: "0 24px 20px", position: "relative", zIndex: 20 }}>
        ...
        >
          ...
          {[15, 30, 70, 88].map((pos, i) => (
            <div
              ...
              style={{
                ...
                bottom: 0,
                width: 1,
                background: `rgba(120,115,105,${state.transformationRules[`TR${i + 1}` as keyof typeof state.transformationRules].fired ? 0.4 : 0.12})`,
              }}
            />
          ))}
          ...
          >
            &Delta;T &asymp; {state.uczF.deltaT.toFixed(1)} years
          </div>
        </div>
